...
// src/app/core/guardias/autenticacion.guard.ts
var autenticacionGuard = () => {
...
var routes = [
  ...
  },
  {
    ...
    loadComponent: () => import("./chunk-XNZRUNK2.js").then((m) => m.InicioSesionComponent)
  },
  {
    path: "registrar",
    canActivate: [noAutenticacionGuard],
    loadComponent: () => import("./chunk-AQ3ESMIL.js").then((m) => m.RegistroComponent)
  },
  {
    path: "",
    canActivate: [autenticacionGuard],
    loadComponent: () => import("./chunk-YE4W36ZM.js").then((m) => m.DisenoPrincipalComponent),
    children: [
      {
        path: "productos",
        loadComponent: () => import("./chunk-OIZLEIRK.js").then((m) => m.ListaProductosComponent)
      }
    ]
  },
  {
...
// src/app/core/interceptores/jwt.interceptor.ts
...
// src/app/estado/productos/productos.reductor.ts
var productosReductor = createReducer(
  estadoInicialProductos,
  // ---- Cargar Productos ----
  on(ProductosAcciones.cargarProductos, (estado) => __spreadProps(__spreadValues({}, estado), {
    cargando: true,
    error: null
  })),
  on(ProductosAcciones.cargarProductosExito, (estado, { resultado }) => adaptadorProductos.setAll(resultado.datos, __spreadProps(__spreadValues({}, estado), {
    cargando: false,
    error: null,
  ...
  on(ProductosAcciones.cargarProductosError, (estado, { error }) => __spreadProps(__spreadValues({}, estado), {
    cargando: false,
    error
  })),
  // ---- Crear Producto ----
  on(ProductosAcciones.crearProducto, (estado) => __spreadProps(__spreadValues({}, estado), {
    cargando: true,
    error: null,
    mensajeExito: null
  })),
  on(ProductosAcciones.crearProductoExito, (estado, { producto }) => adaptadorProductos.addOne(producto, __spreadProps(__spreadValues({}, estado), {
    cargando: false,
    mensajeExito: `Producto "${producto.nombre}" creado exitosamente.`,
    total: estado.total + 1
  }))),
  on(ProductosAcciones.crearProductoError, (estado, { error }) => __spreadProps(__spreadValues({}, estado), {
    cargando: false,
    error
  })),
  // ---- Actualizar Producto ----
  on(ProductosAcciones.actualizarProducto, (estado) => __spreadProps(__spreadValues({}, estado), {
    cargando: true,
    error: null,
    mensajeExito: null
  })),
  on(ProductosAcciones.actualizarProductoExito, (estado, { producto }) => adaptadorProductos.updateOne({ id: producto._id, changes: producto }, __spreadProps(__spreadValues({}, estado), {
    cargando: false,
    mensajeExito: `Producto "${producto.nombre}" actualizado exitosamente.`
  }))),
  on(ProductosAcciones.actualizarProductoError, (estado, { error }) => __spreadProps(__spreadValues({}, estado), {
    cargando: false,
    error
  })),
  // ---- Eliminar Producto ----
  on(ProductosAcciones.eliminarProducto, (estado) => __spreadProps(__spreadValues({}, estado), {
    cargando: true,
    error: null,
    mensajeExito: null
  })),
  on(ProductosAcciones.eliminarProductoExito, (estado, { id }) => adaptadorProductos.removeOne(id, __spreadProps(__spreadValues({}, estado), {
    cargando: false,
    mensajeExito: "Producto eliminado exitosamente.",
    total: estado.total - 1
  }))),
  on(ProductosAcciones.eliminarProductoError, (estado, { error }) => __spreadProps(__spreadValues({}, estado), {
    cargando: false,
    error
  })),
  // ---- Seleccionar Producto ----
  on(ProductosAcciones.seleccionarProducto, (estado, { id }) => __spreadProps(__spreadValues({}, estado), {
    productoSeleccionadoId: id
  })),
  // ---- Limpiar Mensaje ----
  on(ProductosAcciones.limpiarMensaje, (estado) => __spreadProps(__spreadValues({}, estado), {
    error: null,
    mensajeExito: null
  }))
);

// src/app/servicios/productos.servicio.ts
var ProductosServicio = class _ProductosServicio {
  constructor(http) {
    this.http = http;
    this.urlBase = `${ambiente.apiUrl}/productos`;
  }
  /**
   * Obtiene productos paginados con filtros opcionales.
   */
  obtenerProductos(consulta = {}) {
    const parametros = {};
    if (consulta.pagina)
      parametros["pagina"] = consulta.pagina.toString();
    if (consulta.limite)
      parametros["limite"] = consulta.limite.toString();
    if (consulta.categoria)
      parametros["categoria"] = consulta.categoria;
    if (consulta.estado)
      parametros["estado"] = consulta.estado;
    if (consulta.busqueda)
      parametros["busqueda"] = consulta.busqueda;
    return this.http.get(this.urlBase, { params: parametros });
  }
  /**
  ...
  static {
    this.\u0275fac = function ProductosServicio_Factory(t) {
      return new (t || _ProductosServicio)(\u0275\u0275inject(HttpClient));
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ProductosServicio, factory: _ProductosServicio.\u0275fac, providedIn: "root" });
  }
};

// src/app/estado/productos/productos.efectos.ts
var ProductosEfectos = class _ProductosEfectos {
  constructor(acciones$, productosServicio) {
    this.acciones$ = acciones$;
    this.productosServicio = productosServicio;
    this.cargarProductos$ = createEffect(() => this.acciones$.pipe(ofType(ProductosAcciones.cargarProductos), mergeMap(({ consulta }) => this.productosServicio.obtenerProductos(consulta).pipe(map((respuesta) => ProductosAcciones.cargarProductosExito({ resultado: respuesta.datos })), catchError((error) => of(ProductosAcciones.cargarProductosError({
      error: error.error?.mensaje || "Error al cargar los productos."
    })))))));
    this.crearProducto$ = createEffect(() => this.acciones$.pipe(ofType(ProductosAcciones.crearProducto), mergeMap(({ producto }) => this.productosServicio.crearProducto(producto).pipe(map((respuesta) => ProductosAcciones.crearProductoExito({ producto: respuesta.datos })), catchError((error) => of(ProductosAcciones.crearProductoError({
      error: error.error?.mensaje || "Error al crear el producto."
    })))))));
    this.actualizarProducto$ = createEffect(() => this.acciones$.pipe(ofType(ProductosAcciones.actualizarProducto), mergeMap(({ id, producto }) => this.productosServicio.actualizarProducto(id, producto).pipe(map((respuesta) => ProductosAcciones.actualizarProductoExito({ producto: respuesta.datos })), catchError((error) => of(ProductosAcciones.actualizarProductoError({
      error: error.error?.mensaje || "Error al actualizar el producto."
    })))))));
    this.eliminarProducto$ = createEffect(() => this.acciones$.pipe(ofType(ProductosAcciones.eliminarProducto), mergeMap(({ id }) => this.productosServicio.eliminarProducto(id).pipe(map(() => ProductosAcciones.eliminarProductoExito({ id })), catchError((error) => of(ProductosAcciones.eliminarProductoError({
      error: error.error?.mensaje || "Error al eliminar el producto."
    })))))));
  }
  static {
    this.\u0275fac = function ProductosEfectos_Factory(t) {
      return new (t || _ProductosEfectos)(\u0275\u0275inject(Actions), \u0275\u0275inject(ProductosServicio));
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ProductosEfectos, factory: _ProductosEfectos.\u0275fac });
  }
};
...
var appConfig = {
  providers: [
    ...
    ...ambiente.produccion ? [] : [provideStoreDevtools({ maxAge: 25, logOnly: false })]
  ]
};

// src/main.ts
bootstrapApplication(AppComponent, appConfig).catch((err) => console.error("Error al iniciar la aplicaci\xF3n:", err));
